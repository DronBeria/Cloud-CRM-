...
import { Moon, Sun, Menu, ChevronDown, LogOut, Settings } from "lucide-react";
...
const PATH_LABELS: Record<string, string> = {
  "/admin": "Overview", "/admin/clients": "Clients", "/admin/invoices": "Invoices",
  "/admin/services": "Services", "/admin/tickets": "Support", "/admin/products": "Products",
  "/admin/workflows": "Workflows", "/admin/notifications": "Notifications",
  "/admin/integrations": "Integrations", "/admin/gateways": "Gateways",
  "/admin/extensions": "Extensions", "/admin/audit": "Audit Log",
  "/admin/email-logs": "Email Logs", "/admin/settings": "Settings",
};
...
export function AdminNavbar({ onMenuClick, user }: AdminNavbarProps) {
  ...
  const pageLabel = PATH_LABELS[pathname] ??
    (pathname.includes("/clients/") ? "Client Profile" : "Admin");
  ...
  return (
    <header className="sticky top-0 z-50 flex h-14 items-center border-b border-gray-100 bg-white/95 backdrop-blur px-4 gap-3">
      ...
      <div className="flex items-center gap-2 text-sm flex-1 min-w-0">
        <span className="text-gray-400 hidden sm:block font-medium">{process.env.NEXT_PUBLIC_APP_NAME ?? "CloudCRM"}</span>
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 pl-1 pr-2 py-1.5 rounded-lg hover:bg-gray-100 transition-colors ml-1">
              <Avatar className="h-7 w-7 shrink-0">
                <AvatarFallback className="text-[10px] font-bold bg-indigo-100 text-indigo-700">{initials}</AvatarFallback>
              </Avatar>
          ...
          <DropdownMenuContent align="end" className="w-52 shadow-lg border-gray-100">
            ...
            <DropdownMenuSeparator className="bg-gray-100" />
            <DropdownMenuItem className="text-red-600 focus:text-red-600 cursor-pointer text-sm flex items-center gap-2" onClick={handleSignOut}>
