...
function StaffLoginForm() {
  ...
  const onSubmit = async (data: Form) => {
    ...
    try {
      ...
      // Hard redirect — ensures cookies are sent with the new request to middleware
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="space-y-1.5">
        ...
        <Input type="email" placeholder="staff@company.com" {...register("email")} disabled={loading} autoFocus
          className="h-11 bg-slate-800/50 border-slate-700 text-white placeholder:text-slate-500 focus-visible:ring-indigo-500" />
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <Input type={showPass ? "text" : "password"} placeholder="••••••••" {...register("password")} disabled={loading}
            className="h-11 bg-slate-800/50 border-slate-700 text-white placeholder:text-slate-500 focus-visible:ring-indigo-500 pr-10" />
      ...
      <Button type="submit" disabled={loading} className="w-full h-11 bg-indigo-600 hover:bg-indigo-700 text-white font-medium gap-2 border-0">
    ...
    <div className="min-h-screen flex bg-slate-950">
      <div className="hidden lg:flex lg:w-[55%] flex-col justify-between p-14 bg-slate-900 border-r border-slate-800 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-600 text-white"><Shield className="h-5 w-5" /></div>
          <div>
        ...
        <div className="relative z-10 space-y-6">
          ...
          {["Full client and billing management", "Service provisioning and monitoring", "Support ticket resolution", "Platform configuration"].map((item) => (
            <div key={item} className="flex items-center gap-3"><div className="h-1.5 w-1.5 rounded-full bg-indigo-600 shrink-0" /><p className="text-slate-400 text-sm">{item}</p></div>
          ))}
          <div className="flex items-start gap-3 px-4 py-3 bg-indigo-600/10 border border-indigo-500/20 rounded-lg">
            <Lock className="h-4 w-4 text-indigo-400 shrink-0 mt-0.5" />
            <p className="text-indigo-300 text-xs leading-relaxed">Unauthorized access is strictly prohibited.</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 lg:p-14">
        <div className="lg:hidden mb-8 flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-600 text-white"><Shield className="h-4 w-4" /></div>
          <p className="text-white font-bold">Staff Portal</p>
        </div>
        <div className="w-full max-w-sm">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-white">Sign in</h1>
            <p className="text-slate-400 mt-1 text-sm">Staff accounts only. <a href="/login" className="text-indigo-400 hover:text-indigo-300">Client portal →</a></p>
          </div>
